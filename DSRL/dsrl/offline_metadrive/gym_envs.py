...
class SafeMetaDriveEnv_FSRL(SafeMetaDriveEnv): 
    ...
    def step(self, actions):
        obs, reward, terminated, truncated, info = super(SafeMetaDriveEnv_FSRL, self).step(actions)
        
        info["velocity_cost"] = max(0, 1e-2*(info["velocity"]-10.))
        info["cost"] += info["velocity_cost"]
        info["proximity_cost"] = 1e-2*max(0, 0.1-np.min(obs[-240:]))**2
        info["cost"] += info["proximity_cost"]
        
        info["out_of_road_cost"] = self.config["out_of_road_cost"] if info["out_of_road"] else 0
        info["crash_cost"] = self.config["crash_vehicle_cost"] if info["crash"] else 0
        
        if info["max_step"]: 
            truncated = True
        else: 
            truncated = False
        if info["arrive_dest"] or info["crash"] or info["out_of_road"]: 
            terminated = True
        else: 
            terminated = False
