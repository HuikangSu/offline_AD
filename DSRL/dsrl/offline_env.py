...
def wrap_env(
    ...
    def normalize_state(state):
        # # print(f"Initial observation type: {type(state)}, value: {state}")
        # obs, info = state
        # # print(f"second observation type: {type(obs)}, value: {obs}")
        # normalized_array = (obs - state_mean) / state_std  # 对数组部分进行标准化
        ...
        normalized_array = (obs - state_mean) / state_std
        return (normalized_array, info)
...
class OfflineEnvWrapper(gym.Wrapper, OfflineEnv):
    ...
    def reset(self):
        ...
        # print(f"Initial observation type: {type(obs)}, value: {obs}")
