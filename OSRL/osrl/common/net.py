...
class SquashedGaussianMLPActor(nn.Module):
    ...
    def forward(self,
        ...
        # print("deterministic:", type(deterministic))
        if deterministic:
        ...
        if with_logprob:
            ...
            logp_pi = None
        # print("log_std gradient:", self.log_std_layer.weight.grad)
        # print("mu gradient", self.mu_layer.weight.grad)
        # for BEARL only
...
class SquashedGaussianMLPActorforAWAC(nn.Module):
    '''
    A MLP Gaussian actor, can also be used as a deterministic actor
    ...
    def forward(self,
        ...
        if deterministic:
            ...
            pi_action = pi_distribution.rsample()
        # print("Sampled Action (pi_action):", pi_action)
            
        if with_logprob:
            logp_pi = pi_distribution.log_prob(pi_action).sum(axis=-1)
            # print("Log Probability:", logp_pi)
        else:
            logp_pi = None

        # print("mu",mu,"pi_action",pi_action)
        # print("log_std",log_std,"std",std)
        # print("pro_pi",torch.exp(pi_distribution.log_prob(pi_action)))
        # print("log_std gradient:", self.log_std_layer.weight.grad)
        # print("mu gradient", self.mu_layer.weight.grad)
        pi_action = torch.tanh(pi_action)
        if with_distribution:
